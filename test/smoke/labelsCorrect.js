...
/*
    it('TC-017 Name field label =  "1. What is your Hero\'s name?" ', () => {
    ...
    it( 'TC--018 Gender field label =  "2. Please choose a gender."', function () {
        let genderLabel = $(sel.name).getText();
...
*/
    ...
    it('TC-024 Image field label = "5. Upload an image (optional)"', () => {
        let imageLabel = $$(sel.imageLabel)[4].getText();
        expect(imageLabel).toEqual(exp.imageLabel);
    });

    it('TC-025 Submit button label = "Create!" ', () => {
        let submitButton = $(sel.submitButton).getText();
        expect(submitButton).toEqual(exp.buttonLabel);
